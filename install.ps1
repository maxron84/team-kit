...
function Kopiere-Infrastruktur {
    ...
    foreach ($muster in @('*.sh', '*.ps1', '*.cmd')) {
        foreach ($f in (Get-ChildItem (Join-Path $KIT 'entry') -Filter $muster -File -ErrorAction SilentlyContinue)) {
    ...
    foreach ($f in (Get-ChildItem (Join-Path $KIT 'team\prompts') -Filter '*.md' -File)) {
        Kopiere $f.FullName "team/prompts/$($f.Name)" -Immer:$Immer
    }
    foreach ($f in (Get-ChildItem (Join-Path $KIT 'team\tests') -Filter 'test_*.py' -File)) {
    ...
    Kopiere (Join-Path $KIT 'team\tests\conftest.py') 'team/tests/conftest.py' -Immer:$Immer
}
...
if ($Update) {
    ...
    if (Test-Path $zielTests) {
        foreach ($f in (Get-ChildItem $zielTests -Filter 'test_*.py' -File)) {
            if (-not (Test-Path (Join-Path $KIT "team\tests\$($f.Name)"))) { $fremdeTests += $f.Name }
        }
    }
...
if ((Test-Path (Join-Path $cfgDir 'api-key')) -or (Test-Path (Join-Path $cfgDir 'auth-mode'))) {
    ...
    Gelb "      pwsh -File `"$KIT\scripts\team-auth-setup.ps1`""
    Gelb "      (oder gleich die ganze Maschine: pwsh -File `"$KIT\kit-einrichten.ps1`")"
}
