...
[![Regressionstests 1054](https://img.shields.io/badge/Regressionstests-1054-2ea44f?style=flat-square&logo=pytest&logoColor=white)](geteilt/tests)
...
`TEAM.md` und 1054 Regressionstests.
...
kommen die Backlog-Einträge `BL-1`…`BL-207`; was davon behoben ist, steht im
...
├── tests/              119 Testdateien, 1054 Fälle — der Doppelbahn-Harnisch
...
`BL-207` sind fast alle so entstanden. Damit das nicht von der Disziplin
...
- **Im Feld gelaufen, aber schmal aufgestellt.** Der Dauerbetrieb liegt bei
  **einem** Projekt (`Feld A`: 33 Kaskaden, Greenfield, Linux, bash-Bahn);
  ...
  `BL-1`…`BL-207`, von der toten Fixphase über zwei Löcher in der
...
- **Selbstverifikation**: `bash bash/kit-test.sh` installiert das Kit in ein
  Wegwerf-Repo und fährt dort die 1054 Tests — **zweimal**: einmal mit den
