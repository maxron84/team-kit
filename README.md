...
[![Regressionstests 1053](https://img.shields.io/badge/Regressionstests-1053-2ea44f?style=flat-square&logo=pytest&logoColor=white)](geteilt/tests)
...
Ein Befehl, ein kurzes Aufnahme-Interview, danach liegen 175 Dateien im
...
`TEAM.md` und 1053 Regressionstests.
...
**Offen sind 10 Einträge** — keine Reste dieser Version, sondern acht
...
kommen die Backlog-Einträge `BL-1`…`BL-208`; was davon behoben ist, steht im
...
├── tests/              119 Testdateien, 1053 Fälle — der Doppelbahn-Harnisch
...
`BL-208` sind fast alle so entstanden. Damit das nicht von der Disziplin
...
- **Im Feld gelaufen, aber schmal aufgestellt.** Der Dauerbetrieb liegt bei
  **einem** Projekt (`Feld A`: 33 Kaskaden, Greenfield, Linux, bash-Bahn);
  ...
  `BL-1`…`BL-208`, von der toten Fixphase über zwei Löcher in der
...
- **Selbstverifikation**: `bash bash/kit-test.sh` installiert das Kit in ein
  Wegwerf-Repo und fährt dort die 1053 Tests — **zweimal**: einmal mit den
...
**auch für die 175 Dateien, die der Installer im Zielprojekt hinterlässt** — sie
