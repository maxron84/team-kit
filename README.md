...
| **`Feld E`** | Greenfield, Linux, bash-Bahn: Dart/Flutter + SQLite für ein **Android-Tablet** — Neubau, dessen Python/tkinter-Vorgänger (~25.500 LOC, 17 Spec-Dokumente) als reine Lesereferenz danebenliegt | Erste Kaskade geplant (2026-08-23), noch nicht gebaut. `BL-158`…`BL-162` — **fünf Funde**, vier davon vor dem ersten Lauf, gefunden beim Lesen der Kopplungen zwischen Konfiguration, Testläufer und Rollen-Prompt. Der erste Stack ohne pytest: Was das Kit an Python-Annahmen mitträgt, wird hier zum ersten Mal sichtbar |
Ein künftiges Projekt bekommt den nächsten Buchstaben. Aus diesen sechs Quellen
kommen die Backlog-Einträge `BL-1`…`BL-162`; was davon behoben ist, steht im
