...
> **Die Statusspalte beginnt mit einem Merkwort** (`BL-224`) — `offen`,
> `teilweise` oder `erledigt`, danach darf jede Prosa stehen. `offen` und
> `teilweise` zählen als offen. Der Grund ist eine Zahl: Das README nennt, wie
> viele Einträge offen sind, und `geteilt/kit-readme-pruefen.py` rechnet sie
> **hier** nach. Eine Zelle, die mit „Befund 1 erledigt …, Befund 2 bleibt
> offen" beginnt, ist für einen Leser klar und für einen Zähler nicht — der
> Wächter meldet sie deshalb namentlich als **unentscheidbar**, statt zu raten.
...
**Stand 2026-08-28, vierter Teil — sechs Eintraege abgetragen, alle auf beiden
Bahnen und jeder mit gefahrener Gegenprobe.** `BL-197`, `BL-201`, `BL-205`,
`BL-208`, `BL-169` und `BL-202` (samt `BL-209`, das beim Bauen des letzten
herausfiel). Damit stehen noch **vier** offen, und sie zerfallen in
zwei Gruppen, die verschieden behandelt gehoeren.

> **Der Ertrag von `BL-202` war nicht die Umbenennung, sondern ihre Praemisse.**
> Der Eintrag sagt *„Keine Mechanik haengt am Praefix"* — sie sitzt an SECHS
> Stellen ueber beide Bahnen, und in allen sechs ist „nichts gefunden" ein
> GUELTIGER Wert. Die vorgeschlagene Umbenennung haette die Ableitungen also
> nicht kaputt gemacht, sondern **stumm gestellt**: leerer Ledger-Notiztext,
> „noch keine Aussage moeglich" bei der Altlast-Kennzahl, `None` bei
> `kaskade_beginn` — und damit `BL-45` und `BL-27` ohne Stimme. Daneben fiel
> `BL-209` heraus, das **schon heute** trifft: derselbe Aufruf hatte auch den
> ORDNER verdrahtet, obwohl das Interview ihn abfragt. Wer einen Eintrag
> abtraegt, prueft seine Begruendung mit — sie ist eine Beobachtung, kein
> Befund.
...
**Stand 2026-09-03 — acht liegengebliebene Meldungen aus `Feld E` eingereiht
(`BL-215`…`BL-222`).** Sie lagen seit dem 2026-08-29 als Dateien unter
`plans/meldungen/`, ohne je eine Nummer bekommen zu haben: Der Rückkanal hat
sie abgelegt, die Triage ist zwischen zwei Fix-Sitzungen durchgefallen. Sechs
davon sind älter als `BL-210`, das am 2026-08-31 eingereiht wurde — die
Warteschlange wurde also nicht von vorn abgearbeitet, sondern vom Kopf.

> **Das ist derselbe Ausgang, den `BL-210` beschreibt, eine Ebene höher:** Was
> nicht an einer Mechanik hängt, wird übergangen. Eine Meldung wird per
> `kit-melden ablegen` committet, die `BL-n`-Zeile schreibt ein Mensch von
> Hand, und nichts vergleicht die beiden Mengen. **Ein Wächter, der
> `plans/meldungen/*.md` gegen die in `backlog.md`/`backlog-archiv.md`
> verlinkten Dateinamen hält, hätte alle acht am ersten Tag genannt** — er
> gehört auf dieselbe Liste wie der README-Zahlenwächter (`BL-198`).

- **Zwei bilden ein Paar und gehören zusammen gebaut** — `BL-220` und `BL-221`,
  beide `--rollen-abschluss`, beide eine Fehlzuordnung im Kaskadenfeld, beide
  still und beide erst beim Closeout sichtbar.
- **`BL-218` ist der Zwilling von `BL-213`** aus dem anderen Feld: dieselbe
  Stelle, derselbe Irrtum („ein einzelner erklärbarer Ausreißer wird zur
  Aussage über die TABELLE"), und `BL-211` verweist schon darauf. Die drei
  gehören zusammen entschieden, nicht nacheinander.
- **`BL-222` ist die billigste und die mit dem schlechtesten Verhältnis von
  Aufwand zu Schaden** — zwei Zweige in einem Dispatcher gegen ein falsches
  Erfolgssignal auf jedem Buchungsverb.
- **`BL-219` ist keine Arbeit, sondern eine Entscheidung** und als einziger
  Eintrag eine Idee statt eines Fehlers. Sein Fund — der Kaskadenplan ist das
  einzige Artefakt ohne prüfende Gegenrolle — steht unabhängig von der
  Rollenidee, die ihn zutage gefördert hat.

**Noch am selben Tag abgetragen: sieben der acht, dazu `BL-213`.** Im Archiv
begründet: `BL-215`, `BL-216`, `BL-217`, `BL-218`, `BL-220`, `BL-221`,
`BL-222` — und `BL-213`, das mit `BL-218` an derselben Stelle fiel. **Offen
bleibt allein `BL-219`**, und das aus dem Grund, den es selbst nennt: Es ist
eine Entscheidung des Owners, keine Arbeit. `bash bash/kit-test.sh` ist
danach vollständig durchgelaufen.

> **Der Ertrag war auch hier nicht der grüne Lauf, sondern zwei Abbrüche
> davor** — und beide sind alte Bekannte:
>
> - **Die Zahlen des READMEs standen wieder gegen die Installation.** Drei neue
>   Testdateien heben die Dateizahl (181 → 184), die Testdateizahl (125 → 128)
>   und die Fallzahl (1141 → 1178). Der Wächter aus `BL-198`/`BL-208` hat alle
>   drei gefangen, bevor sie behauptet wurden — genau wofür er gebaut ist.
> - **Zwei neue Testfälle waren in einer einbahnigen Ablage rot statt
>   übersprungen**, weil sie `team-status.ps1` ohne Übersprung öffneten. Das ist
>   wörtlich der Ausrutscher, vor dem der Kopf dieser Datei warnt, und er hat
>   auch diesmal genau eine Runde gekostet.

> **Zwei Stellen sind beim Abtragen aufgefallen und stehen NICHT als eigener
> Eintrag da, weil sie zu bestehenden gehören:**
>
> - **Ein grüner Test hat die Fehlbuchung von `BL-221` festgeschrieben.**
>   `test_bl27_vergessener_abschluss.py` verlangte wörtlich `returncode == 0`
>   und trug den Kommentar *„Kein Abbruch: gebucht wird trotzdem, der Mensch
>   entscheidet"*. Der Mensch KANN dort nicht entscheiden — wenn er den Hinweis
>   liest, ist gebucht und archiviert. Dieselbe Bauart wie das `auth == "api"`
>   aus `BL-143`: eine Zusicherung, die den Fehler konserviert, statt ihn zu
>   fangen.
> - **Der Beifang von `BL-215` ist der Rest von `BL-169`.** Beide
>   Red-Team-Briefings und `bootstrap/CLAUDE.md.vorlage` verlangen weiter
>   `xfail` mit `strict=True`, und das Reproducer-Namensmuster trägt weiter
...
**Nachtrag desselben Tages — die vier abtragbaren Feld-B-Einträge sind
abgetragen: `BL-210`, `BL-211`, `BL-212` und `BL-214`** (Begründungen im
[Archiv](backlog-archiv.md)). `BL-212` war dabei fast geschenkt: Der
Phasen-Zeiger aus `BL-217` weiß genau, welche Phase erreicht wurde, und
`BL-212` ist die Frage, ob der Bericht das auch sagt.

> **Beim Abtragen von `BL-214` fiel seine Prämisse.** Der Eintrag beschreibt
> eine Asymmetrie zwischen `frank.ps1` und `ralph.ps1` — nachgemessen fehlte
> der vierte Ausgang auch `frank.sh`. Es war keine **Bahn**-Asymmetrie,
> sondern eine **Rollen**-Asymmetrie auf beiden Bahnen, und ein Fix nur auf der
> pwsh-Seite hätte die bash-Hälfte des Schadens stehen lassen. Dieselbe Lehre
> wie bei `BL-202`: Wer einen Eintrag abträgt, prüft seine Begründung mit.

> **`BL-211` ist bewusst NICHT durch Zurückdrehen des Kit-Satzes abgetragen.**
> Der mit `BL-166` gesenkte Wert ist für das Feld richtig, das ihn ausgelöst
> hat; für das meldende Feld ist er um 33,3 % falsch. **Beide haben recht** —
> sie rechnen gegen verschiedene Verträge ab. Eine zentrale Zahl kann das nicht
> auflösen, also darf ein Projekt sie überschreiben (`TEAM_PREISE`). Das ist
> die einzige der drei vorgeschlagenen Richtungen, bei der kein Feld auf den
> nächsten Kit-Entscheid warten muss.

**Was offen bleibt — fünf Einträge, und bei vieren ist die ENTSCHEIDUNG der
offene Teil, nicht die Arbeit.** Jeder dieser Einträge sagt das selbst:

| Nr | Was fehlt | Warum es hier nicht abgetragen wurde |
|---|---|---|
| `BL-193` | Weg (3), der Lückenfinder | Kostet eine **Formatänderung am Ledger**. Der Eintrag legt fest, dass darüber erst nach `BL-197` Teil (1) zu befinden ist — das ist jetzt der Fall, und die Frage ist damit fällig, nicht erledigt |
| `BL-194` | zwei Bauformen | *„Erst entscheiden, welche der beiden Bauformen gefahren wird, dann bauen"* — wörtlich der Eintrag |
| `BL-204` | Phase 4 in die Bibliothek | Umbau am **Geld ausgebenden** Pfad. In dieser Sitzung ist Phase 4 bereits zweimal geändert worden (`BL-210`, `BL-214`); ein dritter, rein statisch belegter Eingriff wäre genau die Gattung, bei der dieses Kit sonst einen Menschen im Lauf haben will |
| `BL-206` (2) | Rollback-Schnappschuss | Entwurfsfrage. Die Handregel steht bis dahin in `bootstrap/TEAM.md` |
| `BL-219` | Abnahme-Gate | Eine **neue Rolle**, kein Fix — das ist eine Kaskade, keine Sitzungsaufgabe. Der Schnitt, den das Feld empfiehlt, steht im Eintrag |

> **Und eine Beobachtung, die kein eigener Eintrag ist, aber dieselbe Bauart
> hat:** Elf Zeilen dieser Tabelle tragen im Status „erledigt 2026-08-28" und
> stehen trotzdem noch hier statt im Archiv. Wer die Datei nach „was ist offen"
> liest, liest damit 18 Zeilen, von denen 5 gemeint sind. Genau diese
> Unlesbarkeit hat die acht Meldungen aus `Feld E` durchfallen lassen. Sie
> gehören beim nächsten Durchgang verschoben — nicht in dieser Sitzung, weil
> die Statustexte fremde Entscheidungen zusammenfassen und ein falsch
> verschobener Eintrag aus der Arbeitsliste verschwindet.
...
| BL-202 | **Kaskaden-Plandateien heissen `ralph-kaskade-N-<thema>.md`, obwohl sie das ganze Team binden.** Der Name nennt **eine** Rolle; das Dokument bindet alle: Ralph liest die Stufenbloecke, Harry/Marv beziehen ihren Sweep-Fokus aus dem Plankopf, Frank arbeitet gegen dieselben Zusicherungen, der Architekt schreibt das Abschluss-Doc gegen den Stufenbogen, und der Mensch entscheidet an dieser Datei, was ueberhaupt gebaut wird. Keine Mechanik haengt am Praefix — es steht nur in Vorlagen und Anleitungen (`bootstrap/CLAUDE.md.vorlage`, `bootstrap/TEAM.md`, `bootstrap/roadmap-skizzen.md`, `geteilt/prompts/rolle-architekt.md`, die Entrypoints beider Bahnen, `install.sh`, `lib.sh` sowie mehrere `geteilt/tests/` als Beispielwert) | `Feld B`, 2026-08-27, beim Schreiben der fuenften Kaskade. Voller Hergang: [`plans/meldungen/2026-08-27-kaskaden-plandateien-heissen-ralph-kaskade-n-obwohl-sie-das.md`](meldungen/2026-08-27-kaskaden-plandateien-heissen-ralph-kaskade-n-obwohl-sie-das.md) | **erledigt 2026-08-28 — aber ANDERS als vorgeschlagen, weil die Praemisse des Eintrags falsch war.** **Der Eintrag sagt: *„Keine Mechanik haengt am Praefix."* Das stimmt nicht** — sie sitzt an SECHS Stellen ueber beide Bahnen: `kaskade_beginn()` in `kosten.py` (darauf stuetzen sich der Zeitraum-Abgleich aus `BL-45` und die P1b-Pruefung aus `BL-27`), `team_architekt_kaskade` und `team_bau_notiz` in `lib.sh` (`BL-34`), die Altlast-Kennzahl in `team-status.sh`, und die drei pwsh-Gegenstuecke. **Warum das schlimmer ist als ein Fehler:** In allen sechs Faellen ist „nichts gefunden" ein GUELTIGER Wert — eine benannte Kaskade hat keine Nummer, ein frisches Projekt keine Plandatei. Die naive Umbenennung haette die Ableitungen also nicht kaputt gemacht, sondern **stumm gestellt**. **Deshalb war der Fix nicht die Umbenennung, sondern die TOLERANZ:** Alle sechs Stellen erkennen jetzt beide Formen, `kosten.py` haelt sie in `PLAN_PRAEFIXE` an EINER Stelle (`BL-154`). Erst danach ist in den VORLAGEN umbenannt (`bootstrap/`, `rolle-architekt.md`, README, `ralph.sh`-Kopf, `install.sh`-Abschlusstext); Bestandsprojekte behalten ihre Dateinamen, und ein eigener Fall haelt fest, dass die historischen Verweise im Kit-Archiv **nicht** mitumbenannt werden — sie sind die Spur, und die Spur ist hier das Produkt. **Zweiter Fund derselben Zeile, und er trifft SCHON HEUTE (`BL-209`):** `kaskade_beginn()` hatte auch den ORDNER als Literal (`"plans"`), obwohl `TEAM_PLAN_ORDNER` im Interview abgefragt wird. **Jedes Projekt mit einem abweichenden Planordner hat `BL-45` und `BL-27` seither stumm.** Beide `team.config.*` exportieren den Wert jetzt, wie `TEAM_DOMAENEN` daneben. **Am VERHALTEN belegt, nicht nur am Quelltext:** vier Kombinationen aus Praefix und Ordner durch `kaskade_beginn` gefahren — gegen den alten Stand liefert genau EINE einen Zeitstempel, die anderen drei `None`. Dazu die Gegenrichtung: eine benannte Kaskade (`post-20`) muss weiter `None` liefern, sonst datiert der Abgleich auf eine Datei, die die Kaskade gar nicht eroeffnet hat. Regressionstest `test_bl202_plandatei_traegt_beide_praefixe.py`, **23 Faelle**; gegen den alten Stand fallen 20, die drei gruenen sind die Gegenrichtungen |
...
| BL-209 | **`kaskade_beginn()` sucht die Plandatei einer Kaskade in einem verdrahteten Ordner `plans/` — obwohl `TEAM_PLAN_ORDNER` im Aufnahme-Interview abgefragt wird.** `geteilt/tools/kosten.py` baut sein Suchmuster aus zwei Literalen: dem Ordner `"plans"` und dem Dateinamen-Praefix `"ralph-kaskade-"`. Der Ordner ist aber konfigurierbar; das Interview fragt ihn ab, beide `team.config.*` fuehren ihn, und ein Projekt darf dort `team-plans/` oder `docs/plaene/` antworten. **In jedem solchen Projekt findet die Suche NIE eine Plandatei.** Und das ist kein Fehler, sondern **Schweigen**: `kaskade_beginn` gibt dann `None` zurueck, und `None` ist an dieser Stelle ein gueltiger Wert — eine benannte Kaskade hat keine nummerierte Plandatei, ein frisches Projekt gar keine. **Zwei Waechter stuetzen sich darauf und hoeren still auf zu arbeiten:** der Zeitraum-Abgleich aus `BL-45` (er faengt einen Lauf, der ZWISCHEN zwei Kaskaden liegt und sonst falsch beschriftet gebucht wird — im Feld 4,2560 USD) und die P1b-Pruefung aus `BL-27` (unarchivierte Logs, die aelter sind als der Kaskadenbeginn — im Feld 33,89 USD). Beide melden dann nichts mehr, und „nichts gemeldet" liest sich wie „nichts gefunden". **Dieselbe Gattung wie `BL-131`:** eine Maschinen- bzw. Projekteigenschaft als Literal im Werkzeug | Kit, 2026-08-28 — beim Bauen von `BL-202` gefunden. Nicht gesucht: Der Eintrag behauptet, keine Mechanik haenge am Dateinamen-Praefix; beim Nachzaehlen der Fundstellen fiel der Ordner daneben mit auf. **Am Verhalten gemessen, nicht vermutet:** vier Kombinationen aus Praefix und Ordner durch `kaskade_beginn` gefahren — gegen den alten Stand liefert genau EINE einen Zeitstempel (`plans/ralph-kaskade-…`), die anderen drei `None` | **erledigt 2026-08-28, zusammen mit `BL-202`.** Neu ist `plan_ordner()` in `kosten.py`: Es liest `TEAM_PLAN_ORDNER` aus der Umgebung und faellt auf `plans` zurueck — dieselbe Bauart wie `erlaubte_domaenen()` zehn Zeilen darueber, und aus demselben Grund. Beide `team.config.*` **exportieren** den Wert jetzt (`export` bzw. `$env:`), genau wie `TEAM_DOMAENEN` daneben; ohne den Export nuetzt das Ableiten nichts. Das Praefix liegt in `PLAN_PRAEFIXE` an EINER Stelle statt als Literal (`BL-154`). **Gegenprobe gefahren** und in `test_bl202_plandatei_traegt_beide_praefixe.py` festgehalten: dieselben vier Kombinationen muessen jetzt ALLE einen Zeitstempel liefern — und die Gegenrichtung, eine benannte Kaskade (`post-20`) muss weiter `None` liefern, sonst datiert der Abgleich auf eine Datei, die die Kaskade gar nicht eroeffnet hat |
| BL-219 | **Der Kaskadenplan ist das einzige Artefakt des Kits ohne Netz — es fehlt das Abnahme-Gate, nicht eine Bindeglied-Rolle.** Ralphs Produktivcode prüft der Smoke-Test, Franks Fix die Gegenprobe plus Reproducer, Harrys/Marvs Fund die Reproschritte und Franks Nachprüfung, Axels Akte Franks Umsetzung — **den Plan des Architekten prüft nichts**. Die Stufen-`Verifikation` fragt *„funktioniert es"* und ist vom Architekten für seinen eigenen Entwurf geschrieben; die Frage *„ist es das Richtige"* stellt im ganzen Kit niemand außer dem Menschen, und der stellt sie am Ende, aus dem Gedächtnis. Ein falscher Plan wird von Ralph treu ausgeführt und kostet eine ganze Kaskade. Anlass war die Idee einer Product-Owner-Rolle; der Fund dahinter wiegt schwerer als die Idee | Feld E, 2026-08-30, Kit 2.13.1, bash/linux, Ausarbeitung mit dem Stakeholder — **Art: Idee/Verbesserung, nicht Fehler** — Meldung: `plans/meldungen/2026-08-30-product-owner-rolle-automatisieren-als-abnahme-gate-tragfaeh.md` | **offen — und zuerst eine ENTSCHEIDUNG, keine Arbeit.** Der Schnitt, den das Feld empfiehlt: **Abnahme voll automatisieren, Priorisierung nur als Vorschlag mit Begründung** — die beiden Hälften tragen das Risiko völlig ungleich (Fehler bei der Abnahme kostet eine Backlog-Zeile, Fehler bei der Priorisierung eine ganze Kaskade). **Die naive Fassung „PO als Bindeglied zwischen Mensch und Team" hat das Feld selbst verworfen**, mit vier Einwänden, die mitgeschrieben gehören: ein Zwischenglied ersetzt Information statt sie zu verlieren (*ein Modell antwortet immer* — dieselbe Bauart wie ein Smoke-Test, der sich die Umgebung passend setzt); Abstraktion ist Rauschunterdrückung, und im Feld war das Rauschen die Information; Rubber-Stamp-Falle (Entscheidungsrecht bleibt, Entscheidungs*fähigkeit* wandert weg); das Bindeglied existiert schon und heißt Architekt. **Die Eignungsbedingung gehört in jede Fassung, die gebaut wird:** Die Rolle trägt, je fester die Ziele extern verankert sind (Vorbild, nachschlagbar) — am wenigsten taugt sie im Greenfield ohne Vorbild, also genau dort, wo ein Team sie am dringendsten haben will. Nachprüfbar statt Ermessen: *Kann ich auf eine Stelle im Vorbild zeigen? → entscheiden. Nein? → **das** ist die eine Frage an den Menschen.* **Keine neuen Dokumentformate** — Auftrag in den Plankopf, Abnahme in einen Abschnitt des Abschluss-Docs. **Unabhängig davon nützlich und heute nicht im Kit:** „Die Modellstärke folgt der Frage *hat die Ausgabe dieser Rolle ein maschinelles Netz?*, nicht der Frage *ist die Rolle senior?*" — daraus folgt ausdrücklich, den Architekten **nicht** herunterzustufen, sondern kürzer laufen zu lassen |
| BL-232 | **Virenschutz, Suite-Laufzeit und der vierte Ausgang sind EINE Kausalkette — das Kit behandelt sie als drei Einzelfaelle und greift dreimal hinter der Ursache.** (1) Auf Windows kostet jeder Pruefstands-Kindprozess einen Virenscan; der Preis haengt an der Zahl der STARTS (im selben Lauf gemessen: 5 Tests mit 5 Starts = 42 s gegen eine ganze Datei mit einem Start = 13,75 s). (2) Die Suite waechst damit mit der Zahl der STARTS, nicht der Tests — im Feld 294,80 s (422 Tests), 492,79 s (451) und 378,55 s (453) an EINEM Tag — die Suite streut um rund 30 %, eine Rate aus zwei Laeufen ist deshalb KEINE Messgroesse (in der Meldung ausdruecklich widerrufen). (3) **Der Loop faehrt den Verifikationsbefehl bis zu zweimal je Stufe**: einmal von der bauenden Rolle, dann noch einmal von der Selbstpruefung des vierten Ausgangs, auf demselben Stand — bei vier Stufen rund **32 Minuten reine Doppelmessung**. (4) Je laenger die Suite laeuft, desto lastempfindlicher wird sie: zwei Laeufe in Folge endeten im vierten Ausgang, einer davon wegen 16 Fehlschlaegen, die allein gefahren gruen waren. Belegzahl des Laufs: **3 h 03 min Wanduhrzeit fuer 16,19 USD Modellkosten** — ueber zwei Stunden davon Warten. | Feld B, 2026-09-05, Kit 2.13.1, pwsh/win32, zehnte Kaskade, ~450 Tests, Python-Dienst plus Electron-Oberflaeche. **Art: Fehler am Kit.** Drei Vordergrundlaeufe ohne Nebenlast plus ein Lauf mit `--durations=30`. Meldung: `plans/meldungen/2026-09-05-virenschutz-suite-laufzeit-und-der-vierte-ausgang-sind-eine.md` | **offen.** Vier unabhaengige Vorschlaege, nach Wirkung sortiert: **(1) Die Doppelmessung abschaffen** — die Selbstpruefung soll das Ergebnis des Rollenlaufs verwenden statt ihn zu wiederholen, oder die Doppelmessung wenigstens MELDEN; spart ohne jede Projektarbeit die halbe Wartezeit. **(2) Zweistufige Verifikation als Kit-Begriff** (`TEAM_SMOKE_TEST_SCHNELL` neben `TEAM_SMOKE_TEST`): je Stufe schnell, am Phasenende vollstaendig — heute erfindet das jedes Projekt selbst und schwaecht dabei unbemerkt die Stufenverifikation, weil eine Kit-Regel fehlt, wann der volle Lauf verbindlich ist. **(3) Flaky vom Regress trennen, bevor der Mensch geweckt wird** — bei rotem Baum die roten Dateien allein wiederholen und den Unterschied berichten; genau diese Handbewegung loeste den Feldfall auf, und sie ist mechanisch. **(4) Frühwarnung statt Deckel** — `TEAM_SMOKE_TEST_TIMEOUT` (Default 600 s, aus `BL-207`) warnt bei ~75 % und nennt die Dauer; im Feld blieben 107 s Puffer bei einem Tageswachstum von 198 s. **Wichtig fuer das Triage:** Die AV-Ausnahme ist KEINE brauchbare Kit-Antwort — auf einer verwalteten Firmeninstallation hat das Baukonto keinen Zugriff auf den Virenschutz (`Get-MpPreference` -> `0x800106ba`). Punkt (3) beruehrt `BL-207` und die vierte Fehlerklasse, ersetzt aber keinen der beiden. |
| BL-233 | **Die `.gitignore`-Vorlage kennt `.ralph-state`, aber nicht `.vollautomatik-state` — der Guard warnt deshalb bei JEDEM Rollenstart.** Die Vollautomatik schreibt ihren Phasenzustand waehrend des Laufs in eine Datei, die die Vorlage nicht ignoriert; ihre drei Geschwister (`.ralph-logs/`, `.team-logs/`, `.ralph-state`) stehen dort sehr wohl. Ab dem ersten Phasenwechsel ist der Arbeitsbaum damit dreckig, und der Guard fordert `bitte committen` — was mitten im eigenen Lauf weder moeglich noch sinnvoll ist, weil der naechste Phasenwechsel die Datei erneut schreibt. **Die Vollautomatik verstoesst zwangslaeufig gegen die Sauberkeitsregel ihres eigenen Guards.** | Feld B, 2026-09-05, Kit 2.13.1, pwsh/win32, zehnte Kaskade. Meldung: `plans/meldungen/2026-09-05-die-gitignore-vorlage-kennt-ralph-state-aber-nicht-vollautom.md` | **offen.** Der Guard behandelt den Fall danach korrekt (kein Rollback, 'nicht dieser Rolle zugeschrieben') — **der Schaden ist die Entwertung der Warnung, nicht der Rollback.** Feldbeleg aus demselben Lauf: Beim zweiten Red-Team-Sweep standen zwei Pfade in der Warnung, die strukturelle Zustandsdatei und eine ECHTE Fremdaenderung (eine zweite Instanz hatte im Plan-Ordner geschrieben). Der echte Fall stand neben dem Dauerfall und ging in ihm unter; bemerkt wurde er erst spaeter beim Lesen des Protokolls — genau davor soll die Warnung schuetzen. **Drei Vorschlaege:** (1) die eine Zeile in die Vorlage, neben `.ralph-state`. (2) **Bestandsprojekte mitnehmen** — eine reine Vorlagenaenderung erreicht sie NICHT, dort liegt die Datei bereits im Index und `.gitignore` greift dann nicht mehr; `--update` sollte das erkennen und `git rm --cached` vorschlagen. (3) Grundsaetzlich: Der Guard koennte die Zustandsdateien des Kits selbst von der Sauberkeitspruefung ausnehmen, statt sich auf eine vollstaendige `.gitignore` zu verlassen — er kennt ihre Namen ohnehin. Verwandt mit `BL-232`: beide Male arbeitet ein Kit-Mechanismus gegen einen anderen Kit-Mechanismus. |
| BL-234 | **`zitat_lint.py` kennt nur `BL-`Nummern — Fund- und Aktenzitate (`HM-`, `AX-`) veralten unsichtbar.** Das Werkzeug liest ausschliesslich den Backlog als Statusquelle (`REFERENZ_RE = (?<!Kit-)BL-(\d+)`). Das Beutebuch und die Ermittlungsakten fuehren dieselbe Statuskette, liegen im selben Plan-Ordner und werden in Plaenen und Skizzen genauso als offene Frage zitiert — der Lint sieht sie nicht. Im Feld begruendete eine Roadmap-Skizze ihre **harte Vorbedingung** mit drei Fund-Nummern, die eine Fixphase derselben Kaskade wenige Stunden zuvor erledigt hatte; der Lint meldete fuenf andere, korrekt erkannte `BL-`Faelle und diesen nicht. Ein Fund ist dabei die WAHRSCHEINLICHERE Sperre, weil er den Loop stilllegt, und Fund-Zitate veralten schneller, weil eine Fixphase mehrere Funde in einer Nacht erledigt. Fuer das Beutebuch existiert mit `team/tools/beutebuch.py` bereits ein Statusleser. | Feld B, 2026-09-07, Kit 2.13.1, pwsh/win32, elfte Kaskade. Meldung: `plans/meldungen/2026-09-07-zitat-lint-py-kennt-nur-bl-nummern-fund-und-aktenzitate-hm-a.md` | **offen.** Kein Heuristik-Problem, sondern eine fehlende Quelle — die bewusste Schmalheit des Lints (nur Zukunftsform, Rueckblicke fallen heraus) traegt unveraendert weiter. Vorschlag in der Meldung: zweites Praefix-Paar mit dem vorhandenen Beutebuch-Leser als Statusquelle, `Kit-HM-` analog zu `Kit-BL-` ausgeschlossen. |
| BL-235 | **Die Laufzeit-Ausgaben nennen Kit-Backlog-Nummern blank (`BL-41`, `BL-135`), obwohl `CLAUDE.md`, `TEAM.md` und sogar `zitat_lint.py` die `Kit-BL-`Regel aus `BL-140` befolgen.** Der Abbruchbericht des vierten Ausgangs druckt woertlich 'STUFE FERTIG, QUITTUNG FEHLT (BL-41)' und 'Keine Datei unter tests/ beruehrt (BL-135)'. Im Feldprojekt ist `BL-41` ebenfalls vergeben, fuer ein voellig anderes Thema — wer der Nummer im eigenen Backlog nachschlaegt (die naheliegende Handlung: der eigene Backlog ist der offene), landet in einem unverwandten Eintrag ohne jeden Hinweis auf die Verwechslung. Betroffen ist die Stelle, an der ein Mensch die Nummer unter Zeitdruck liest: der Bericht einer gerade gescheiterten Stufe. Die niedrigen Kit-Nummern in den Ausgaben (`BL-41`, `BL-135`, `BL-205`, `BL-207`) liegen genau im Bereich, den ein gewachsenes Feldprojekt selbst vergeben hat. | Feld B, 2026-09-07, Kit 2.13.1, pwsh/win32, elfte Kaskade. Meldung: `plans/meldungen/2026-09-07-laufzeit-ausgaben-nennen-kit-backlog-nummern-blank-bl-41-obw.md` | **offen.** `BL-140` hat die Regel in den Regeltexten abgetragen, die Laufzeit-Ausgaben aber nicht mitgezogen. Kein Einzelfix: Die Fehlerklasse ist der blanke Nummernraum in JEDER gelesenen Ausgabe, ein Rundblick ueber `BL-<N>` in den Ausgabetexten von `team/lib.*` und den Entrypoints duerfte weitere Stellen zeigen. |
| BL-236 | **Der Rollback der Fixphase setzt hart auf einen beim Start gemerkten Commit zurueck und wirft fremde Commits weg, die waehrenddessen entstanden sind.** Waehrend ein Fix-Lauf arbeitete, legte eine zweite, interaktive Sitzung im selben Arbeitsbaum einen reinen Doku-Commit an (Fundliste, keine Datei des Fix-Laufs beruehrt). Nach dem Lauf war er aus der Historie verschwunden: Der Reflog zeigt `reset: moving to <gemerkte Basis>` ueber den fremden Commit hinweg. **Nichts hat es gemeldet** — der Lauf endete mit seiner normalen Erfolgsmeldung, und der Verlust traf genau den Eintrag, den die zweite Sitzung hinzugefuegt hatte. Zurueckgeholt per `git cherry-pick` aus dem Reflog. | Feld B, 2026-09-07, Kit 2.13.1, pwsh/win32, elfte Kaskade. Meldung: `plans/meldungen/2026-09-07-der-rollback-der-fixphase-setzt-hart-auf-einen-gemerkten-com.md` | **offen.** Der Loop ist ausdruecklich dafuer gebaut, dass ein Mensch daneben weiterarbeitet — ein Rollback, der die Spitze der Historie fuer seine eigene haelt, ist mit dieser Arbeitsteilung unvereinbar. Der Verlust ist still und asymmetrisch: Die eigenen Commits schreibt der Loop korrekt, nur die des Menschen verschwinden. Trifft doppelt, wenn der verlorene Eintrag ein FUND war: `first 'an Frank uebergeben'` findet ihn nicht mehr, der naechste Lauf meldet 'nichts zu tun'. Vorschlag: vor dem harten Zuruecksetzen pruefen, ob HEAD noch die gemerkte Basis oder ein selbst angelegter Commit ist; ist die Spitze fremd, den Fall an den Menschen geben statt zurueckzurollen. |
| BL-237 | **Der Bestandsvermerk der Schreibzone wird beim Einzug erfasst und altert danach still — was das Projekt SPAETER dort ablegt, sieht niemand.** `BL-51` (erledigt, 2.6.0) deckt den Einzug ab und benennt die Grenze selbst: *'--update urteilt NUR ueber den Config-Vermerk, nie ueber den Ordner.'* Zwischen zwei Updates kann sich der Plan-Ordner beliebig fuellen. Im Feld lagen nach elf Kaskaden **neun** Nicht-Plan-Dateien in der Schreibzone, darunter vier ausfuehrbare Pruefskripte (79 KB) und ein vom Stakeholder ausdruecklich als verbindliche Richtlinie deklarierter Gestaltungsvertrag — alle aenderbar und loeschbar durch die Read-Only-Rollen, weil der Vermerk beim Einzug (Greenfield) zu Recht LEER war und leer blieb. **Dasselbe Muster bei `TEAM_WEITERER_CODE`, mit umgekehrter Folge:** Dieselben vier Skripte standen nicht darin und wurden nie gesweept — der `BL-52`-Fall, nicht beim Einzug entstanden, sondern hineingewachsen. Ein Ordner, den es beim Einzug nicht gab, kann in keiner Einzugspruefung auffallen. | Feld B, 2026-09-07, Kit 2.13.1, pwsh/win32, elfte Kaskade. Aufgefallen durch eine beilaeufige Frage des Stakeholders nach der Ordnerstruktur — nicht durch ein Werkzeug, nicht durch einen Sweep, nicht durch --update. Meldung: `plans/meldungen/2026-09-07-der-bestandsvermerk-der-schreibzone-wird-beim-einzug-erfasst.md` | **offen.** Zwei Einmal-Werte, die mit dem Projekt altern, beide lautlos; der zweite ist der gefaehrlichere, weil er die AUSSAGE des Sweeps verfaelscht statt nur eine Datei zu gefaehrden. Vorschlag ausdruecklich OHNE Guard-Mechanik (die Begruendung aus `BL-51` traegt weiter: auf der Whitelist kann der Guard nicht urteilen): Der Statusbericht nennt beilaeufig, was in der Schreibzone liegt und weder Team-Artefakt (Beutebuch, Backlog, Roadmap, Kaskadenplaene, Abschlussprotokolle, Akten) noch im Bestandsvermerk verzeichnet ist — ein Hinweis, kein Abbruch, keine Meinung ueber die Zugehoerigkeit. Dieselbe Zeile koennte ausfuehrbare Dateien ausserhalb von Produktivordner und Pruefumfang melden. |
| BL-238 | **`kosten.py sitzung-messen` laeuft ueber den DOKUMENTIERTEN Direktaufruf ungeeicht — `TEAM_PREISE` erreicht es nur ueber die Shell-Konfiguration.** Das Rollen-Briefing des Architekten nennt an zwei Stellen `python3 team/tools/kosten.py sitzung-messen --projekt .` als den Weg. `TEAM_PREISE` wird aber ausschliesslich von `team.config.ps1`/`team.config.sh` exportiert; ein Direktaufruf laedt sie nie. Ergebnis: *'Preistabelle stimmt nicht mehr: 159 von 161 nachgerechneten Laeufen weichen ab'*, alle um konstant 33,3 % (Faktor 1,5). Mit vorangestelltem `TEAM_PREISE=…`: *'geeicht an 161 abgerechneten Laeufen'* — gleiche Sitzung, gleiche Ausgabezahl, gegensaetzliche Buchbarkeit. | Feld B, 2026-09-07, Kit 2.13.1, pwsh/win32, zwoelfte Kaskade. Aufgefallen beim Kostenabschluss: Die Regel *ungeeicht → nicht buchen* blockierte den Abschluss, fuer den der Aufruf dokumentiert ist. Meldung: `plans/meldungen/2026-09-07-kosten-py-sitzung-messen-laeuft-ueber-den-dokumentierten-dir.md` | **offen.** Die Diagnose zeigt auf die Preistabelle, waehrend der Fehler im Aufrufweg sitzt — sie empfiehlt, `kosten.py` nachzuziehen oder `TEAM_PREISE` zu setzen, und beides ist im Projekt bereits geschehen. Die wahrscheinliche Reaktion ist deshalb ein zweiter Patch in `kosten.py`, also genau der, den das naechste `--update` ueberbuegelt (`BL-42`/`BL-58`) — die Schleife, aus der `BL-211` herausfuehren sollte. Der stille Ausgang ist schlimmer: Wer die Warnung ueberliest, bucht eine um den Tabellenfaktor verschobene Zahl. **Vorschlag (a), tragfaehiger:** `kosten.py` liest die Projekt-Konfiguration selbst, wenn `--projekt` gegeben ist. **(b):** Briefing auf den Wrapper umstellen — `sitzung-messen` ist das einzige Verb ohne einen, daher tritt es genau hier auf. |
| BL-239 | **`--rollen-abschluss` bricht HALB ab und meldet, es habe gar nicht gehandelt.** Die Alterspruefung aus `BL-221` fand drei vorkaskadliche Rollen-Logs und meldete woertlich *'es wird NICHTS gebucht und NICHTS archiviert'*. Tatsaechlich war die `ralph`-Haelfte da bereits gebucht und `.ralph-logs` bereits archiviert. Der zweite Aufruf traf deshalb einen leeren Ordner, wollte 0.0000 USD ueber die bestehende Zeile schreiben und brach mit einer Kollisionsmeldung ab, die den Vorfall nicht erklaeren kann. Symmetrisch ebenso: Der zweite Aufruf meldet fuer `ralph` 'Es wird NICHTS geschrieben' und schreibt die `roles`-Zeile. | Feld B, 2026-09-07, Kit 2.13.1, pwsh/win32, zwoelfte Kaskade. Die Altersprüfung selbst war richtig und wertvoll — die drei Logs stammten aus einer Out-of-Loop-Runde und haetten die Kaskade sonst mit fremden Kosten belastet. Meldung: `plans/meldungen/2026-09-07-rollen-abschluss-bricht-halb-ab-und-meldet-es-habe-gar-nicht.md` | **offen.** Der gutartige Ausgang war Glueck, nicht Konstruktion: Der Kollisionsschutz fing die 0.0000-Ersetzung ab. Die Auswahlhilfe legt dabei sogar den falschen Griff nahe (*'Wurde seit der Altzeile NICHT archiviert, … dann ist --ersetzen richtig'*) — hier WURDE archiviert, aber genau das kann man nach der ersten Meldung nicht wissen. **Vorschlag (b), die eigentliche Reparatur:** beide Quellen erst pruefen, dann buchen; ein Befund haelt den ganzen Aufruf an, bevor etwas geschrieben oder archiviert wird — also das, was der Text heute schon behauptet. **(a):** Die Meldung sagt, fuer welche Haelfte sie gilt. Out-of-Loop-Fixe zwischen zwei Kaskaden sind der Normalfall, die Pruefung trifft also fast jede gewachsene Installation. **GREIFT MIT `0af1347` INEINANDER (Meldung vom 2026-09-06, anderes Feldprojekt, Bash-Bahn): `--rollen-abschluss vor-N` bucht den GESAMTEN unarchivierten Bestand, weil der `BL-221`-Riegel bei benannten Nummern nicht greift.** Zusammen ergeben die beiden Befunde eine Falle, die keiner von beiden allein bildet — und man laeuft genau dann hinein, wenn man der Warnung **woertlich** folgt: (1) `--rollen-abschluss <N>` blockiert wegen Altlogs, bucht aber die `ralph`-Haelfte und archiviert sie, waehrend die Meldung 'NICHTS gebucht, NICHTS archiviert' sagt. (2) Man folgt der Empfehlung und ruft `--rollen-abschluss vor-N` — das bucht nun ALLE unarchivierten Rollen-Logs unter die Vorkaskaden-Nummer, also auch die der gerade fertigen Kaskade. **Ergebnis: Die Kaskade hat eine Bau-Zeile ohne Rollen-Zeile, die Vorkaskaden-Nummer traegt saemtliche Rollenkosten, und `vor-N` bekommt zusaetzlich eine 0.0000-Bau-Zeile, weil der Ralph-Ordner aus Schritt (1) schon leer ist.** Vier Zeilen, jede fuer sich plausibel, keine Warnung, Belege archiviert. Im hier gemeldeten Fall ist das nur ausgeblieben, weil ich die Altlogs VOR Schritt (2) von Hand beiseitegelegt hatte — aus Vorsicht nach einem Blick in den Code, nicht auf einen Hinweis des Werkzeugs hin. **Fuer die Reparatur heisst das:** Die beiden Meldungen einzeln zu beheben genuegt, aber wer nur eine behebt, laesst die Kaskade bestehen. |
| BL-240 | **Der gedruckte Lauf-Betrag und die Turn-Aufzaehlung darunter beschreiben verschiedene Mengen.** Der Abschlussbericht druckt *'Dieser Lauf: 18.8551 USD'* und listet zwei Zeilen darunter *'6 Lauf/Laeufe, 279 Turns'* mit sechs Ralph-Logs — deren Summe ist 15.4039. Der Betrag stammt aus der Lauf-Buchhaltung (alle Rollen dieses Prozesses), das Profil aus dem Ralph-Log-Ordner (was dort liegt, egal aus welchem Lauf). Bei einer ueber ZWEI Laeufe gebauten Kaskade enthaelt der Ordner zusaetzlich die Stufe des ersten Laufs, dafuer fehlt ihm jeder Rollen-Lauf. Keine der beiden Zahlen ist der ganze Lauf, keine die ganze Kaskade (hier 21.9071). | Feld B, 2026-09-07, Kit 2.13.1, pwsh/win32, zwoelfte Kaskade, in zwei Laeufen gebaut. Meldung: `plans/meldungen/2026-09-07-der-gedruckte-lauf-betrag-und-die-turn-aufzaehlung-darunter.md` | **offen.** Der zweite Lauf ist kein Sonderfall, sondern eingeplantes Verhalten: Der vierte Ausgang und der Abbruch am Lauf-Deckel enden beide mit 'von Hand quittieren, dann erneut starten'. Der Bericht ist zugleich die einzige Stelle mit einer Gesamtzahl — wer die falsche der beiden nimmt, bucht 15.40 oder 18.86 statt 21.91, und beide stehen zwei Zeilen auseinander ohne Hinweis auf den Mengenwechsel. Verschaerfend: 'N Laeufe, M Turns' liest sich wie eine Aufschluesselung des Betrags darueber, gehoert aber einer anderen Menge an. **Vorschlaege:** (a) der Betragszeile ihre Menge geben; (b) die Aufzaehlung nach Startzeit des Laufs filtern (die Altersprüfung aus `BL-221` kann das schon) und die Rollen-Laeufe mit auflisten; (c) eine zweite Zeile fuer die KASKADE ueber alle Laeufe. |
| BL-241 | **Die Fixphase hat keinen Einzeleinstieg — wer nur fixen will, zahlt zwei Sweeps oder faehrt Frank ohne Axel.** Phase 4 (Frank fixt -> eskaliert nach drei Fehlversuchen selbst an Axel -> Axel liefert die Akte -> Frank holt den Fund mit `Fix-Plan liegt vor` zurueck) ist vorhanden und funktioniert, aber nur als Teil eines vollen Laufs erreichbar. `vollautomatik` faehrt davor die **Phasen 2 und 3, zwei bezahlte Red-Team-Sweeps**, die neue Funde erzeugen und damit genau das Ziel verschieben, das man abraeumen wollte. `frank` von Hand nimmt EINEN Fund pro Aufruf; in einer Schleife fehlt **Axel** — nach drei Fehlversuchen setzt Frank den Fund selbst auf `an Axel uebergeben` und findet ihn beim naechsten Aufruf nicht mehr, die Schleife meldet dann 'nichts zu tun', waehrend ein Fall auf Axel wartet. **`TEAM_VOLLAUTOMATIK_AB_PHASE` sieht wie die Loesung aus und ist keine:** Der Name verspricht 'ab Phase N', der Code prueft ausschliesslich auf `'1'`, und `=4` wird **still ignoriert** — der naheliegende Griff fuehrt lautlos zum teuersten Ergebnis. | Feld B, 2026-09-08, Kit 2.13.1, pwsh/win32, zwoelfte Kaskade. Nach einer Handabnahme lagen drei Funde mit Status 'an Frank uebergeben' ausserhalb einer Kaskade (Bau abgeschlossen, .ralph-state ueber RALPH_CAP). Meldung: `plans/meldungen/2026-09-08-die-fixphase-hat-keinen-einzeleinstieg-wer-nur-fixen-will-za.md` | **offen.** Out-of-Loop-Funde zwischen zwei Kaskaden sind der Normalfall, nicht die Ausnahme — das Kit hat fuer sie eigens Franks Dreisatz, den Status `an Frank uebergeben` und mit `--kaskade vor-N` sogar eine eigene Kostenzeile; fuer ihr ABARBEITEN gibt es keinen Weg. Die stille Variante ist die teure: Wer `frank` in eine Schleife setzt, bekommt eine Fixphase ohne Axel und merkt es nicht. **Vorschlag klein:** `TEAM_VOLLAUTOMATIK_AB_PHASE` die Werte 2/3/4 annehmen lassen (die der Phasen-Zeiger ohnehin kennt), unbekannte Werte abbrechen statt still auf Phase 1 fallen. **Vorschlag gross und aus Sicht des Melders der eigentliche Punkt:** ein eigener Entrypoint `fixphase`, der Phase 4 samt Lauf-Deckel, Stagnations-Bremse und Abschlussbericht faehrt. 'Mehrere offene Funde abarbeiten, ohne eine Kaskade zu starten' ist heute die einzige Team-Taetigkeit ohne eigenen Aufrufer — ralph, harry, marv, frank und axel haben je einen. Im Feldprojekt lokal mit einem Projektskript geloest, das nur die regulaeren Entrypoints ruft und Axel nur dann, wenn das Beutebuch einen Fall fuer ihn fuehrt; bewusst ohne Deckel und Bremse, weil beides ins Kit gehoert. |
