...
**Dazu vier Funde aus `Feld E`** (`BL-158`…`BL-161`), dem ersten Zielprojekt
...
die stillschweigend Python heißt.** `BL-158` (Ordner-Defaults und
Reproducer-Namensmuster gegen paketgebundene Testsuche) ist der Kern; `BL-160`
(die Kit-Selbsttests verdrahten `.py` und `strict=True`) ist seine Spiegelseite
in der eigenen Suite. `BL-159` ist die vierte Einsetzstelle von
`{{SMOKE_TEST}}`, die der Abtrag von `BL-149` nicht mitgezählt hat — ein
Rollen-Briefing, das kein Mensch gegenliest. `BL-161` ist unabhängig davon: Der
...
> Vorgaben waren dort schlicht richtig. `BL-158` und `BL-160` teilen sich
