...
`BL-164`, `BL-165` und `BL-166` kamen am selben Tag aus dem Feld dazu und sind
...
- **`BL-166`** — und der Rest davon: `CLAUDE.md` lässt sich **nicht**
...
- **`BL-168`** — und beim Aufräumen fiel auf, dass die **Selbsttests selbst**
  noch stumm liefen: `kit-test.ps1` 14 Minuten, `kit-test.sh` zweimal in
  Stufe 8 (der schwersten, ~55 min). `BL-165` hatte nur die Installer
  getroffen. Jetzt prüft ein Wächter die Gattung statt der Stellen.
- **`BL-167`** (offen, beim Bauen von `BL-166` gefunden) — `install.ps1` hat
...
Ausführungsrichtlinie aus dem Feld) und `BL-167` (der Abgleich-Block fehlt der
