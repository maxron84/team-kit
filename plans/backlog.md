...
| BL-227 | **Der Hilfetext von `kit-melden.ps1` kennt `ablegen` nicht — also genau den Weg, den ein Owner gehen soll.** Der Kopfkommentar zählt fünf Verben auf (`neu`, `pruefen`, `senden`, `issue-link`, `kit-pfad`); `kit_meldung.py` kennt sechs. `ablegen` ist vorhanden, funktioniert und hat einen ausführlichen Docstring — nur die Stelle, die ein Mensch zuerst liest, weiß nichts davon. Wer das Werkzeug über seine eigene Hilfe erschließt, findet `senden` und damit den Pull Request. **Es ist der Rest einer abgetragenen Aufgabe:** `BL-187` („Der Rückkanal kennt nur einen Weg — den Pull Request — und der ist für den OWNER der falsche") steht im Archiv als abgetragen; nachgezogen wurden Rollen-Briefing, `bootstrap/TEAM.md` und `bootstrap/CLAUDE.md.vorlage`, der Hilfetext des Werkzeugs selbst nicht. Erschwerend: Die Ausgabe von `neu` nennt `ablegen` — wer über `neu` einsteigt, findet es; wer die Datei öffnet, nicht | **Feld B**, 2026-09-03, Kit 2.13.1, pwsh/win32, neunte Kaskade, Owner-Feld — Meldung: `plans/meldungen/2026-09-03-der-hilfetext-von-kit-melden-kennt-das-verb-ablegen-nicht-al.md` | **offen, klein.** Verbliste um `ablegen` ergänzen und in einem Halbsatz sagen, wann man es nimmt („liegt das Kit lokal daneben, ist `ablegen` der Weg; `senden` ist für Melder ohne Kit-Repo") — derselbe Satz, den `neu` schon ausgibt. Der `WARUM SENDEN EINE EIGENE HANDLUNG IST`-Absatz sollte `ablegen`s eigene, strengere Grenzen mitnennen (kein Push, keine `BL-`Nummer, Redaktionsprüfung als Vorbedingung). **Bash-Bahn mitprüfen** — das meldende Feld fährt nur pwsh |
| BL-228 | **Ein Netzfehler VOR dem ersten Token zählt bei der Fixer-Rolle als inhaltlicher Fehlversuch und schiebt den Fund Richtung Axel — obwohl nie ein Modell den Fund gesehen hat.** Abo-Aufruf und API-Fallback endeten beide mit `API Error: Connection refused — a firewall or proxy may be blocking it (ConnectionRefused)`, `terminal_reason: api_error`, **0 Turns, 0.0000 USD**. `frank.*` wertete das als inhaltlichen Fehlversuch, machte Rollback und schrieb den Fund in den Fehlversuchszähler. Nach drei solchen Netzaussetzern stünde er auf `an Axel übergeben`. **Die Unterscheidung existiert bereits, sie ist nur zu eng gezogen:** Der Session-Limit-Zweig (Exit 42) nimmt den Zähler ausdrücklich aus — „kein inhaltlicher Fehlversuch, die Rolle kam nie zum Zug". Ein `ConnectionRefused` mit 0 Turns ist exakt dieselbe Klasse, fällt aber in den generischen Exit 1. **Dritte Variante desselben Musters aus diesem Feld** neben Exit 42 und `BL-214` (Asymmetrie zwischen bauender und fixender Rolle) | **Feld B**, 2026-09-03, Kit 2.13.1, pwsh/win32, Firmennetz mit Pflicht-Proxy — Meldung: `plans/meldungen/2026-09-03-ein-netzfehler-vor-dem-ersten-token-zaehlt-als-inhaltlicher.md` | **offen.** **Vorschlag aus der Meldung — ein Begriff statt drei Einzelzweige:** *Was 0 Turns und 0.0000 USD kostet, ist nie ein inhaltlicher Fehlversuch.* Maschinell prüfbar und deckt alle drei Varianten ab. **Belegter Behelf im Feld:** Rollenläufe mit gesetztem Proxy starten — zehn Aufrufe, kein einziger 0-Token-Fehlversuch (der Lauf davor hatte zwei). Trägt, ist aber Handdisziplin. **Beweislage:** Die Rohlogs sind inzwischen weg (Logordner gitignoriert, beim Kostenabschluss zusätzlich archiviert); die Zahlen stammen aus dem Backlog-Eintrag, der sie beim Auftreten festhielt |
| BL-229 | **Ein zitiertes `#`+`>` in einem Block-Kommentar zerlegt `pwsh/lib.psm1` — und damit die GANZE pwsh-Bahn, unbemerkt.** Der Kopfkommentar von `Team-HilfeKopf` (neu mit `BL-223`) nennt im Fliesstext die beiden Zeichen, zwischen denen ein Block-Kommentar steht. PowerShell kennt darin **keine Maskierung**: Das zitierte Endezeichen schliesst den Kommentar an Ort und Stelle. Alles darunter wird Code, die naechste Prosazeile beginnt mit `$PSCommandPath` — `Unexpected token 'auf'`, und `Import-Module ./team/lib.psm1` scheitert. **Damit laeuft auf der pwsh-Bahn nichts mehr**: keine Rolle, kein Entrypoint, kein Werkzeug — alle laden dieses Modul als erste Handlung. **Warum es niemand sah:** Der Fix zu `BL-223` wurde mit `bash bash/kit-test.sh` abgenommen, und die bash-Bahn ist von einem kaputten `lib.psm1` nicht betroffen; auf einem Wirt ohne pwsh ueberspringen die pwsh-Faelle mit Begruendung. Rot wird nichts — sichtbar wird der Schaden nur dort, wo die Bahn benutzt wird. **Die Fehlerklasse ist aelter als der Fund:** Eine Zeile aus lauter blanken Woertern haette PowerShell klaglos als Kommandoaufruf geparst; erst die Variable in der uebernaechsten Zeile machte den Bruch zu einem Syntaxfehler. Dasselbe Zitat an einer anderen Stelle waere also still geblieben. | Feld B, 2026-09-04, Kit 2.13.1, pwsh/win32 — gefunden beim Einreihen von `BL-227`/`BL-228`: Die Suite meldete **98 rote Faelle, alle auf der pwsh-Bahn, keinen auf der bash-Bahn** (1140 gruen, 40 uebersprungen). Die Sonde aus `test_bl182` sagte, dass jeder Konfigurationswert die Modulgrenze nicht ueberlebt; der Grund lag eine Ebene tiefer. Nachgestellt mit `[System.Management.Automation.Language.Parser]::ParseFile` gegen jede `.ps1`/`.psm1` der Ablage: **genau eine Datei** parst nicht. Eingeschleppt mit `c1a73b1` (`BL-223`) | **offen, und es eilt.** Der Fix selbst ist eine Zeile — die Wendung darf das Endezeichen nicht zitieren. Der Ertrag liegt im **Waechter**: Jede ausgelieferte PowerShell-Datei muss vom PowerShell-Parser angenommen werden. Das ist die billigste denkbare Zusicherung (kein Lauf, kein Repo, kein Modellaufruf) und faengt jeden Syntaxbruch, nicht nur diesen. **Zweiter Teil, schwerer:** Ein Fix, der nur auf einer Bahn abgenommen wird, kann die andere totlegen, ohne rot zu werden — `BL-208` hat dieselbe Bauform fuer die Zahlen beschrieben. |
