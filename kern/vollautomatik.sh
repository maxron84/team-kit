...
while [ "$runde" -lt "$MAX_RUNDEN" ]; do
    ...
    getan=0
    fortschritt=0
    VORHER="$(python3 scripts/beutebuch.py list)"
    ...
    if python3 scripts/beutebuch.py first "an Axel übergeben" >/dev/null; then
        ./axel.sh; rc=$?
        case "$rc" in
    ...
    NACHHER="$(python3 scripts/beutebuch.py list)"
    ...
    if [ "$stagnation" -ge "$STAGNATION_MAX" ]; then
        STUCK="$(python3 scripts/beutebuch.py first 'an Frank übergeben' 2>/dev/null \
            || python3 scripts/beutebuch.py first 'Fix-Plan liegt vor' 2>/dev/null \
            || python3 scripts/beutebuch.py first 'an Axel übergeben' 2>/dev/null \
            || echo '?')"
