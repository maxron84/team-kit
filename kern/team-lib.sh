...
team_diff_beruehrt_fund() {
    ...
    fund_dateien="$(python3 scripts/beutebuch.py dateien "$hm" 2>/dev/null || true)"
...
team_kosten_summe() {
    python3 scripts/kosten.py summe "$@"
}
...
team_kosten_seit() {
    local seit="$1"; shift
    python3 scripts/kosten.py summe --since "$seit" "$@"
}
...
team_kosten_split() {
    python3 scripts/kosten.py summe --split "$@"
}
...
team_ledger_summe() {
    python3 scripts/kosten.py ledger "${1:-.budget-ledger}"
}
...
team_ledger_domaene() {
    ...
    python3 scripts/kosten.py ledger "$pfad" --domaene "$domaene"
}
...
team_ledger_split() {
    python3 scripts/kosten.py ledger "${1:-.budget-ledger}" --split
}
...
team_akteur_abschluss() {
    ...
    if [ -n "$notiz" ]; then
        python3 scripts/kosten.py akteur-abschluss --usd "$usd" \
...
team_architekt_stand() {
    ...
    if [ -n "$kaskade" ]; then
        anzahl="$(python3 scripts/kosten.py ledger "$ledger_pfad" --rolle architekt --kaskade "$kaskade" --anzahl 2>/dev/null)"
        if [ -n "$anzahl" ] && [ "$anzahl" -gt 0 ] 2>/dev/null; then
            echt="$(python3 scripts/kosten.py ledger "$ledger_pfad" --rolle architekt --kaskade "$kaskade" 2>/dev/null)"
            printf '%s\techt\n' "$echt"
            return 0
        fi
    fi
...
team_architekt_schaetzung() {
    ...
    python3 scripts/kosten.py architekt-schaetzung --since "$ref" 2>/dev/null \
        || echo "0.0000"
}
