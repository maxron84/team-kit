...
naechster_schritt() {
    ...
    if $TEAM_BEUTEBUCH_TOOL first "Fix-Plan liegt vor" >/dev/null \
       || $TEAM_BEUTEBUCH_TOOL first "an Frank übergeben" >/dev/null; then
        echo frank; return
    fi
    if $TEAM_BEUTEBUCH_TOOL first "an Axel übergeben" >/dev/null; then echo axel; return; fi
    echo fertig
}
