...
# python: Abhaengigkeit der TEAM-Infrastruktur (team/tools/), nicht des
...
$cfgDir = Team-CfgDir
$authSetup = Join-Path $KIT 'scripts\team-auth-setup.ps1'
...
if ($NurPruefen) {
    ...
    Verknuepfe (Join-Path $KIT 'scripts\team-init.ps1')       'team-init.cmd'
    Verknuepfe (Join-Path $KIT 'scripts\team-auth-setup.ps1') 'team-auth-setup.cmd'
    ...
    Zeile "    pwsh -File `"$KIT\install.ps1`" <zielpfad>"
}
...
if ($Ziel -and -not $NurPruefen) {
    ...
    Kopf "Weiter"
    Zeile "Einbinden in ein Projekt:  pwsh -File `"$KIT\install.ps1`" <zielpfad>"
