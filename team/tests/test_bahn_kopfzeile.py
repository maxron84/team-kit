...
GETEILT = ("team/tools/beutebuch.py", "team/tools/kosten.py",
           "team/tools/zitat_lint.py")
...
def _ist_kit():
    ...
    return (WURZEL / "entry").is_dir() and (WURZEL / "kit-test.sh").is_file()
...
def test_gegenstueck_existiert_und_liegt_auf_der_anderen_bahn():
    ...
    for p in _dateien():
        ...
        if gegenstueck.startswith("keines"):
            continue
        ziel = p.parent / gegenstueck
        if not ziel.is_file():
            schlecht.append(f"{_rel(p)} nennt `{gegenstueck}` — die Datei "
                            f"liegt nicht neben ihr")
            continue
