...
@pytest.mark.parametrize("skript", [
    "entry/frank.sh", "entry/axel.sh", "team/redteam.sh",
    "entry/frank.ps1", "entry/axel.ps1", "team/redteam.ps1",
])
def test_kein_einstiegsskript_rollt_mehr_blanko_zurueck(skript):
    ...
    pfad = wurzel / skript
    if not pfad.is_file():                      # installierte Ablage: Wurzel
        pfad = wurzel / Path(skript).name
    if not pfad.is_file():
...
def test_frank_lauf_verschont_fremde_arbeit(tmp_path):
    ...
    if not frank.is_file():
        pytest.skip("frank.sh liegt nur in der INSTALLIERTEN Ablage "
                    "(im Kit unter entry/) — geprueft wird via kit-test.sh")
