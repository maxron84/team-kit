...
def _fixture_repo(tmp_path):
    ...
    shutil.copy(REPO_ROOT / "scripts" / "kosten.py", tmp_path / "scripts" / "kosten.py")
...
def test_vollstaendiger_aufruf_traegt_korrekte_zeile_ein(tmp_path):
    ...
    rc, out, err = _run(repo, "frank", "abo", "12.34", "website", "Fix-Serie K15")
    assert rc == 0, err
    ...
    assert frank_zeilen[0]["domaene"] == "website"
...
def test_zweiter_aufruf_ersetzt_statt_verdoppelt(tmp_path):
    repo = _fixture_repo(tmp_path)
    _run(repo, "frank", "abo", "10", "website", "erster Versuch")
    rc, out, err = _run(repo, "frank", "abo", "15.5", "website", "korrigiert")
    assert rc == 0, err
